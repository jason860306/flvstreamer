...
/**
 * @brief 完整的Chunk结构，用来保存Raw Chunk数据
 */
typedef struct RTMPChunk
{
  int c_headerSize; ///< Basic Header + Chunk Msg Header的大小
  int c_chunkSize;  ///< Chunk payload length, default 128 bytes
  char *c_chunk;    ///< Chunk payload
  char c_header[RTMP_MAX_HEADER_SIZE];  ///< Chunk Header, (Basic Header + Chunk Msg Header)
} RTMPChunk;
  ...
  1.1.1. Chunk basic header 1
   0 1 2 3 4 5 6 7
  +-+-+-+-+-+-+-+-+
  |fmt|   cs id   |
  +-+-+-+-+-+-+-+-+

  1.1.2. Chunk basic header 2
   0                   1
...
{
  BYTE m_headerType;        ///< Chunk - FMT
  BYTE m_packetType;        ///< Chunk Msg Header - Message Type ID
  BYTE m_hasAbsTimestamp;	// timestamp absolute or relative?
  int m_nChannel;           ///< Chunk - chunk stream ID (aka cs id)
  uint32_t m_nInfoField1;	///< Chunk Msg Header - Timestamp // 3 first bytes
  int32_t m_nInfoField2;	///< Chunk Msg Header - Message Stream ID // last 4 bytes in a long header, absolute timestamp for long headers, relative timestamp for short headers
  uint32_t m_nTimeStamp;	// absolute timestamp
  uint32_t m_nBodySize;     ///< Chunk Msg Header - Message Length, Chunk Payload的大小
  uint32_t m_nBytesRead;
  RTMPChunk *m_chunk;
  char *m_body;             ///< 包括完整的Chunk Header内容
                            ///< (Basic Header + Chunk Msg Header + [Extended Time Stamp +] Chunk Payload)
} RTMPPacket;
...
typedef struct RTMP
{
  ...
  /**
   * @brief 在处理4种类型的块消息头（Chunk Header）时保存的一个块流中
   * 之前所接收到的Chunk Header，用于后来接收到的不是完整的Chunk Header
   * 时，可以直接从之前收到的Chunk Header中取得Chunk Header组成完整信息
   */
