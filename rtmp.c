...
/**
 * @brief 表示四种Chunk Msg Header的大小，取决于Chunk/Basic Header/csid的取值
 */
...
RTMPPacket_Alloc(RTMPPacket * p, int nSize)
{
    ...
    if (!ptr)
        return false;
    p->m_body = ptr + RTMP_MAX_HEADER_SIZE; ///< 包括完整的Chunk Header内容
    p->m_nBytesRead = 0;
    return true;
}
...
RTMP_ReadPacket(RTMP * r, RTMPPacket * packet)
{
    ...
    packet->m_nChannel = (hbuf[0] & 0x3f);  ///< csid: 6bit
    header++;
    if (packet->m_nChannel == 0)
    {
        /**
         * @brief csid=0 代表Basic Header总共要占用2个字节，CSID在[64，319]之间，其值为（第二个字节+64）
         */
        if (ReadN(r, &hbuf[1], 1) != 1) ///< 读取剩下的1个字节
        {
    ...
    {
        /**
         * @brief csid=1 代表Basic Header总共占用3个字节，CSID在[64-65599]，其值为（第三个字节*256+第二个字节+64）
         */
        int tmp;
        if (ReadN(r, &hbuf[1], 2) != 2) ///< 读取剩下的2个字节
        {
            Log(LOGERROR, "%s, failed to read RTMP packet header 3nd byte",
                __FUNCTION__);
            return false;
        }
        tmp = (((unsigned) hbuf[2]) << 8) + (unsigned) hbuf[1]; ///< 左移8位相当于乘以2^8=256，此处为第三个字节*256+第二个字节
    ...
    /**
     * @brief 接收并处理完Chunk的Basic Header后，继续接收后面的Chunk Message Header
     */
    ...
    hSize = nSize+(header-hbuf); ///< Basic Header + Chunk Msg Header的大小

    if (nSize >= 3)
    {
        packet->m_nInfoField1 = AMF_DecodeInt24(header); ///< Chunk Msg Header - Timestamp // 3 first bytes

        Log(LOGDEBUG, "%s, reading RTMP packet chunk on channel %x, "
                      "headersz %i, timestamp %i, abs timestamp %i",
            __FUNCTION__, packet.m_nChannel, nSize, packet.m_nInfoField1,
            packet.m_hasAbsTimestamp);

        if (nSize >= 6)
        {
            packet->m_nBodySize = AMF_DecodeInt24(header + 3); ///< Chunk Msg Header - Message Length
            packet->m_nBytesRead = 0;
            RTMPPacket_Free(packet);    ///< 释放packet的m_body

            if (nSize > 6)
            {
                packet->m_packetType = header[6]; ///< Chunk Msg Header - Message Type ID

                if (nSize == 11)
                    packet->m_nInfoField2 = DecodeInt32LE(header + 7); ///< Chunk Msg Header - Message Stream ID
            }
        }
        /**
         * @brief 对于0类型的块。消息的绝对时间戳在这里发送。如果时间戳大于或等于
         *        16777215（16进制0x00ffffff），该值必须为16777215，并且扩展时
         *        间戳必须出现。否则该值就是整个的时间戳。
         */
        if (packet->m_nInfoField1 == 0xffffff)
        {
            ...
            packet->m_nInfoField1 = AMF_DecodeInt32(header+nSize); ///< 从Chunk - Extended Timestamp取得扩展的时间戳
            hSize += 4;
        }
    }
    ...
    int nChunk = r->m_inChunkSize;  ///< Chunk大小，默认为128字节
    if (nToRead < nChunk)
        nChunk = nToRead;
...
{
    /**
     * @brief clientbuf: 握手阶段客户端的chunk buffer，包括C0、C1
     *        clientsig: C1 chunk
     *        serversig: S1 chunk
     *        client2:   C2 chunk
     *
    ...
    /**
     * @brief RTMP协议中规定的客户端C0字段代表RTMP协议版本
     * @details clientsig[-1] = clientsig - 1
     *
     * 0 1 2 3 4 5 6 7
     * +-+-+-+-+-+-+-+-+
     * |    version    |
    ...
    memcpy(clientsig, &uptime, 4);  ///< C1 - time字段 (4 bytes)

    memset(&clientsig[4], 0, 4);    ///< C1 - zero字段 (4 bytes)

    /**
     * @brief random bytes字段 (1536 - 8 = 1528 bytes)
     */
#ifdef _DEBUG
    /**
     * @brief 简单的将random字段全部置为0xff
     */
    for (i = 8; i < RTMP_SIG_SIZE; i++)
        clientsig[i] = 0xff;
#else
    /**
     * @brief 按4字节为单位用rand置成实际的随机数
     */
    for (i = 8; i < RTMP_SIG_SIZE; i++)
    ...
    /**
     * @brief client发送C0 + C1开始向server端进行握手
     */
    if (!WriteN(r, clientbuf, RTMP_SIG_SIZE + 1))
        return false;

    /**
     * @brief 接收server端对C0的回包，即接收S0包
     */
    char type;
    ...
    /**
     * @brief 接收server端对C1的回包，即接收S1包，大小为1536字节
     */
    ...
    suptime = ntohl(suptime);   ///< S1 - time字段 (4 bytes)
     ...
     * @brief 直接用server端回复的S1当作C2进行client端的第2步握手，即发送C2
     */
...
RTMP_SendPacket(RTMP * r, RTMPPacket * packet, bool queue)
{
    ...
    /**
     * @brief 1. Chunk/Basic Header - fmt & csid
     */
    ...
    *hptr++ = c;
    ...
    /**
     * @brief 1. Chunk/Basic Header - 设置csid的值，当fmt取不同的取值时
     */
    if (cSize)
    {
    ...
    {
        /**
         * @brief Type 0/1/2时编码Timestamp字段，因为Type 4没有Timestamp字段
         */
    ...
    {
        /**
         * @brief Type 0/1时编码Chunk Msg Header - Message Length字段，
         * 因为Type 2/3没有Chunk Msg Header - Message Length字段
         */
    ...
    {
        /**
         * @brief Type 0时编码Chunk Msg Header - Message Stream ID字段，
         * 因为Type 1/2/3没有Chunk Msg Header - Message Stream ID字段
         */
    ...
    {
        /**
         * @brief Type 0/1/2时且Chunk Msg Header中的Timestamp大小0xffffff
         * 编码Extended Timestamp字段，因为Type 3没有Timestamp字段，因此
         * Extended Timestamp字段也不能包含
         */
    ...
    while (nSize+hSize)
    {
        ...
        /**
         * @brief 设置发送的Chunk大小，指Chunk Payload数据的大小
         */
        if (nSize < nChunkSize)
            nChunkSize = nSize;
        ...
        {
            /**
             * @brief 当Chunk Header存在时，一次性将Chunk Header + Chunk Payload全部发送出去
             */
        ...
        {
            /**
             * @brief 对要发送的数据包按Chunk Size进行拆包，每个拆包都要
             * 重新封装包头，如此循环发送，直到将数据全部发送完成
             */
            header = buffer - 1;
            hSize = 1;
            if (cSize)
